...
async function pullPlans(api, opts = {}) {
  ...
  for (const p of list) {
    ...
    const full = path8.join(plansDir(), filename);
...
async function pushPlanFile(api, file, opts = {}) {
  ...
  const relPath = path8.relative(homeBase(), file);
  const state = await readState();
...
// apps/antigravity-plugin/src/hooks/session-start.ts
var CONTEXT_NOTE = [
...
async function main() {
  ...
  if (!ctx) {
    ...
    return emitContext([CONTEXT_NOTE]);
  }
  const { api, config, workspaceBound } = ctx;
  let resolved;
  try {
    resolved = await resolveProject(api, cwd, config.project_id);
  } catch (err) {
  ...
  const active = isWorkspaceActive({ resolvedProjectId: resolved.project_id, workspaceBound });
  if (!active) {
    const idleBanner = await buildSessionBanner(null, { authenticated: true });
    return emitContext([CONTEXT_NOTE, idleBanner]);
  }
  try {
    const state = await readState();
    await applyPullToLocal([], state, (/* @__PURE__ */ new Date()).toISOString());
    const planResult = await pullPlans(api, { projectId: resolved.project_id });
    setLastPlanPullCursor(state, (/* @__PURE__ */ new Date()).toISOString());
    await writeState(state);
    const reconcile = await reconcileKnownPlans(api, { cwd }).catch((err) => {
    ...
    log(
      `session-start: ${parts.length ? parts.join(", ") : "nothing changed"} (via ${resolved.reason})`
    );
  } catch (err) {
    log(`session-start sync failed: ${err instanceof Error ? err.message : String(err)}`);
  }
  let accountName = `${config.account_id.slice(0, 8)}\u2026`;
  try {
  ...
  try {
    handoffContext = await acceptPendingHandoffContext(api, resolved.project_id);
  } catch {
  }
  let unboundNudge = null;
  try {
  ...
  emitContext([CONTEXT_NOTE, banner, handoffContext, unboundNudge, inboxNudge]);
}
