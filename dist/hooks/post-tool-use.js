...
function statePath(cwd) {
  ...
  return path8.join(os6.tmpdir(), `memlin-antigravity-heartbeat-${key}.json`);
}
...
async function maybePushPlan(ctx, payload) {
  ...
  if (!abs.startsWith(plansDir + path9.sep) || !abs.endsWith(".md")) return;
  try {
  ...
  try {
    ...
    log(
      `pushed plan ${path9.basename(abs)} (${result.created ? "new" : "v" + result.version_number})`
    );
  } catch (err) {
