...
var RESOLVE_BIN = path3.resolve(__dirname, "../cli/resolve.js");
var PULL_PLANS_BIN = path3.resolve(__dirname, "../cli/pull-plans.js");
...
async function main() {
  ...
  try {
    ...
    if (state.last_resolve && isContinuation(prompt, cwd, "antigravity", state.last_resolve, payload.session_id ?? null)) {
      emitAdditionalContext(scribeNotice + buildContinuityMarker(state.last_resolve.audit_id));
      process.exit(0);
    }
  } catch {
  }
  const lateBundle = await takePendingBundle(cwd, "antigravity");
  firePlanSync(cwd);
